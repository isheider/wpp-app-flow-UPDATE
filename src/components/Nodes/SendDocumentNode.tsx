...
const DocumentNodeComponent = memo(() => {
  ...
  const addDocument = useCallback(async (dc: any) => {
    setDocument(dc);
  }, []);

  const downloadDocument = useCallback(async (uri: string, name: string) => {
  ...
  return (
    <>
      {doc ? (
        <>
          <div
            onClick={() => downloadDocument(URL.createObjectURL(doc), doc.name)}
            className="nodrag cursor-pointer flex justify-between items-center w-full p-3 rounded-lg align-middle bg-gray-200 text-base font-medium"
          >
            <span>{doc.name}</span>
            <FaDownload size={20} className="text-gray-500 text-base" />
          </div>
          <button
            type="button"
        ...
        <div className="flex mt-4 items-center justify-center w-full">
          <label className="flex flex-col items-center justify-center w-full h-48 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
            ...
            <input
              multiple={false}
              id="dropzone-file"
              onChange={(e: any) => addDocument(e.target.files[0])}
              type="file"
              accept="image/*"
              className="hidden"
            />
    ...
    <ContextMenu.Trigger>
      <div className="w-96 bg-white border border-gray-200 rounded-lg shadow p-[25px] dark:bg-gray-800 dark:border-gray-700">
        ...
        <NodeHeader
          ...
          color="bg-cyan-900"
          message="Envie um documento para o usuário"
        />
